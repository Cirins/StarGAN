...
def calculate_classification_scores(syn_data, syn_labels, syn_doms, src_class, trg_classes, step, mode, 
    ...
    for domain in np.unique(syn_doms):
        ...
        save_classification_scores(classification_scores, src_class, trg_class, domain, step, mode, eval_dir, num_train_domains)
...
def save_classification_scores(classification_scores, src_class, trg_class, domain, step, mode, eval_dir, num_train_domains):
    # Ensure the directory exists
    os.makedirs(eval_dir, exist_ok=True)
    # Path to the CSV file
    ...
    with open(file_path, mode='a' if file_exists else 'w', newline='') as file:
        ...
        # If the file does not exist, write the header
        if not file_exists:
            writer.writerow(['step', 'mode', 'source', 'target', 'domain', 'accuracy', 'loss'])

        accuracy, loss = classification_scores
        # Write the data rows
        writer.writerow([step, mode, src_class, trg_class, domain+num_train_domains, accuracy, loss])
